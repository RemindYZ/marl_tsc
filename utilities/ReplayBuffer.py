...
class Replay_Buffer():
    ...
    def add_experience(self, states, actions, rewards, next_states, local_memorys, dones):
        if type(dones) == list:
            assert type(dones[0]) != list, "A done shouldn't be a list"
            experiences = [self.experience(state, action, reward, next_state, local_memory, done)
                           for state, action, reward, next_state, local_memory, done in
                           zip(states, actions, rewards, next_states, local_memorys, dones)]
    ...
    def sample(self, num_experience):
        ...
        if separate_out_data_types:
            ...
            return states, actions, rewards, next_states, local_memorys, dones
        else:
            return experiences
